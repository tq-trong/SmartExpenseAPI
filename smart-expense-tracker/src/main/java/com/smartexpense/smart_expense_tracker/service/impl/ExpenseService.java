...
public class ExpenseService implements IExpenseService {
    ...
    @Override
    public List<ExpenseDTO> getExpenses(String userFilter, LocalDate startDate, LocalDate endDate, String category, String search, Pageable pageable) {
        ...
        return expensesPage.getContent().stream()
    ...
    public ExpenseDTO updateExpense(String expenseId, ExpenseDTO dto) {
        ...
        if(expense.getUser().equals(user)) {
            ...
            userService.createLog(user.getUsername(), user.getUsername() + LogAction.UPDATED.getAction() + "expense " + expense.getId());
    ...
    public void deleteExpense(String expenseId) {
        ...
        Expense expense = expenseRepository.findById(expenseId)
                .orElseThrow(() -> new AppException(ErrorCode.EXPENSE_NOT_EXISTED));
        if(expense.getUser().equals(user)) {
            userService.createLog(user.getUsername(), user.getUsername() + LogAction.DELETED.getAction() + "expense "+ expense.getId());
            expenseRepository.delete(expense);
        } else throw new AppException(ErrorCode.PERMISSION_INVALID);
    }

    @Override
    public long totalExpenses(String userFilter, LocalDate startDate, LocalDate endDate, String category, String search, Pageable pageable) {
        User user = userRepository.findByUsername(userConverter.toEntity(userService.getMyInfo()).getUsername())
                .orElseThrow(() -> new AppException(ErrorCode.USER_NOT_EXISTED));
        Family family = familyRepository.findByUser(user.getUsername())
                .orElseThrow(() -> new AppException(ErrorCode.FAMILY_NOT_EXISTED));
